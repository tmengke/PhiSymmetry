...
void cetaflatHEP1t(int nIterN=1, double Ethr1=4, double Ethr2=150) {
 ...
 TFile *fila = new TFile (ctit);
  ...
  TH1F *hcorr1Derr_18 = new TH1F("hcorr1Derr_18","Corr",300,0.,2.);
  TH1F *hcorr1Derr_21 = new TH1F("hcorr1Derr_21","Corr",300,0.,2.);
  TH1F *hcorr1Derr_28 = new TH1F("hcorr1Derr_28","Corr",300,0.,2.);
  ...
  TH1F *spectra17 = new TH1F("spectra17","HEP17",1000,0.,250.);
  TH1F *spectra17n = new TH1F("spectra17n","!HEP17",1000,0.,250.);
  ...
  for (int ii=0;ii<14;ii++) {
    ...
    for (int jj=0;jj<72;jj++) {
      ...
      if (abs(ieta)==16) continue;                                                                                                                                                   if (abs(ieta)==29) continue;                                                                                                                                             
      if (abs(ieta)>20 && iphi%2==0) continue;
  ...
  cgL->Print("pictHEplot/phiProfHEP1.gif");
    ...
    FILE *ft1;
  sprintf(ctit,"TEST/HEP1.txt",ftit,((int) Ethr1),((int) Ethr2));
  ...
  for (int ii=0;ii<14;ii++) {
   ...
   if (abs(ieta)==16) continue;
    ...
    for (int jj=0;jj<72;jj++) {
      ...
      if (fabs(ieta) == 18)   {
	hcorr1D_18->Fill(corrL);
	hcorr1Derr_18->Fill(dcorrL);
	//cout<<"filling....."<<endl;
      }


      if (fabs(ieta) == 21)   {
        hcorr1D_21->Fill(corrL);
	hcorr1Derr_21->Fill(dcorrL);
        //cout<<"filling....."<<endl;                                                                                                                                                     
      }

      if (fabs(ieta) == 28)   {
        hcorr1D_28->Fill(corrL);
	hcorr1Derr_28->Fill(dcorrL);
        //cout<<"filling....."<<endl;                                                                                                                                                     
      }
     
      //if ((ieta<=25)&&(ieta>=18)){
       if ((ieta == 17)||(ieta == 26)||(ieta ==27)||(ieta == 28)) {
		if ((ieta!=16)&&(ieta!=29)){
 ...
 fprintf(ft1,"%2d   %2d   1 %8.4f %8.4f\n",ieta,iphi,corrL,dcorrL);
  ...
  TCanvas *c1corr_err_18 =new TCanvas("c1corr_err_18","c1corr_err_18",30,30,600,600);
  hcorr1Derr_18->Draw("hist"); histStat(hcorr1Derr_18,1);
  ...
  TCanvas *c1corr_err_21 =new TCanvas("c1corr_err_21","c1corr_err_21",30,30,600,600);
  hcorr1Derr_21->Draw("hist"); histStat(hcorr1Derr_21,1);
  ...
  TCanvas *c1corr_err_28 =new TCanvas("c1corr_err_28","c1corr_err_28",30,30,600,600);
  hcorr1Derr_28->Draw("hist"); histStat(hcorr1Derr_28,1);
  ...
  TCanvas *chmapppp17 = new TCanvas("chmapppp17","chmapppp17",0,0,650,600);
  chmapppp17->cd(); chmapppp17->SetRightMargin(0.12);


  TLegend *legend17 = new TLegend(0.4,0.6,0.89,0.89);

    Double_t norm = 1;
  Double_t scale = norm/(spectra17->Integral());
  //spectra17->Scale(scale);
  spectra17->Rebin(10);                                                                                                                                                                                 


  spectra17->Draw();

  scale = norm/(17*spectra17n->Integral());
  spectra17n->Scale(1/17.);

  spectra17n->Rebin(10);

   spectra17n->Draw("same");

   legend17->AddEntry(spectra17,"HEP17", "l");
   legend17->AddEntry(spectra17n,"!HEP17", "l");
   //spectra17->Rebin(10);
   ...
   TCanvas *chmapppp17ratio = new TCanvas("chmapppp17r","chmapppp17r",0,0,650,600);

   chmapppp17ratio->cd(); chmapppp17ratio->SetRightMargin(0.12);
   // Define the ratio plot
   TH1F *spectra17r = (TH1F*)spectra17->Clone("spectra17r");
   spectra17r->SetLineColor(kBlack);
   spectra17r->SetMinimum(0.);  // Define Y ..
   spectra17r->SetMaximum(2.2); // .. range
   spectra17r->Sumw2();
   //h3->SetStats(0);      // No statistics on lower plot
   spectra17r->Divide(spectra17n);
   spectra17r->SetMarkerStyle(21);
   spectra17r->Draw("ep");  
   ...
   norm = 1;
   scale = norm/(spectra26n->Integral());
   // spectra26n->Scale(scale);     
    spectra26n->Scale(1/17.);
   spectra26n->Rebin(10);

   spectra26n->Draw("same");

   legend26->AddEntry(spectra26,"ieta = +20, phi = 59", "l");
   legend26->AddEntry(spectra26n,"ieta = +20, phi!=59", "l");
   legend26->Draw();
   ...
   TCanvas *chmapppp27ratio = new TCanvas("chmapppp27r","chmapppp27r",0,0,650,600);

   chmapppp27ratio->cd(); chmapppp27ratio->SetRightMargin(0.12);
   // Define the ratio plot                                                                                                                                                                                 
   TH1F *spectra27r = (TH1F*)spectra27->Clone("spectra27r");
   spectra27r->SetLineColor(kBlack);
   spectra27r->SetMinimum(0.);  // Define Y ..                                                                                                                                                              
   spectra27r->SetMaximum(2.2); // .. range                                                                                                                                                                 
   spectra27r->Sumw2();
   //h3->SetStats(0);      // No statistics on lower plot                                                                                                                                                   
   spectra27r->Divide(spectra27n);
   spectra27r->SetMarkerStyle(21);
   spectra27r->Draw("ep");
   ...
   TCanvas *chmapppp28ratio = new TCanvas("chmapppp28r","chmapppp28r",0,0,650,600);

   chmapppp28ratio->cd(); chmapppp28ratio->SetRightMargin(0.12);
   // Define the ratio plot                                                                                                                                                                                                                                                                                                                                                                                           
   TH1F *spectra28r = (TH1F*)spectra28->Clone("spectra28r");
   spectra28r->SetLineColor(kBlack);
   spectra28r->SetMinimum(0.);  // Define Y ..                                                                                                                                                           \
                                                                                                                                                                                                            
   spectra28r->SetMaximum(2.2); // .. range                                                                                                                                                                                                                                                                                                                                                     
   spectra28r->Sumw2();
   //h3->SetStats(0);      // No statistics on lower plot                                                                                                                                                  
                                                                                                                                                                                                            
   spectra28r->Divide(spectra28n);
   spectra28r->SetMarkerStyle(21);
   spectra28r->Draw("ep");
